...
class SerialApp:
    def __init__(self, root):
        ...
        # Dictionary to store port and baud rate
        self.serial_config = {
            "port": None,
            "baud_rate": 9600
        }
    ...
    def init_excel_file(self):
        if self.excel_file_path:
            ...
            self.sheet.append(["Time (s)", "Seconde", "Temperature", "Voltage", "Current", "Power"])
            self.workbook.save(self.excel_file_path)

    def log_data_to_excel(self, time_val, temp, volt, curr, power):
        if self.excel_file_path:
            # Convert the elapsed time to an absolute timestamp
            absolute_time = self.start_time + time_val
            ...
            # Format the elapsed time as seconds since the start of data collection
            elapsed_time_seconds = int(time_val)

            # Append data with both timestamp formats
            self.sheet.append([formatted_time, elapsed_time_seconds, temp, volt, curr, power])
            self.workbook.save(self.excel_file_path)
    ...
    def start_serial(self):
        ...
        if not self.serial_config["port"]:
            ...
            if not ports:
                ...
                return
            self.serial_config["port"] = ports[0]  # Use the first found port if not set

        if self.serial_thread and self.serial_thread.is_alive():
            self.update_status("Already connected.")
            return

        self.serial_thread = SerialReader(self.serial_config["port"], self.serial_config["baud_rate"], self.update_plot)
    ...
    def update_status(self, message):
        self.status_label.config(text=f"Status: {message}", fg="green" if "Connected" in message else "red")

    def set_port(self, port):
        self.serial_config["port"] = port
        self.update_status(f"Port set to {port}")
...
if __name__ == "__main__":
    root = tk.Tk()
    app = SerialApp(root)
    app.set_port("COM10")  # Set the port externally
    root.mainloop()
